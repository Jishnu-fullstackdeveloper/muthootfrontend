...
const GroupRole: React.FC<GroupRoleProps> = ({ searchText = '', view }) => {
  ...
  // Reset data when searchText changes
  useEffect(() => {
    setPage(1)
    setHasMore(true)
    setAllData([])
  ...
  }, [dispatch, page, limit, isLoadingMore, hasMore, view])
        ...
        <>
          {view === 'table' ? (
            ...
            <>
              ...
              {(isLoadingMore || isGroupRoleLoading) && (
                <Box sx={{ display: 'flex', justifyContent: 'center', my: 4 }}>
                  <CircularProgress />
                </Box>
              )}
              {!hasMore && filteredRoles.length > 0 && (
                <Box sx={{ textAlign: 'center', my: 2 }}>
                  <Typography color='textSecondary'>No more group roles to load</Typography>
                </Box>
              )}
            </>
          )}
